...
if __name__ == "__main__":
    ...
    parser.add_argument('--prevent_use_previous_results_at_each_step',   type=bool, default=False,
    ...
    if not dataset_has_null_values:
        print(f"Dataset doesn't contain null values")
        exit()
    else:
        print(f"Extracting the complete instance of the dataset")
        train_set, test_set = dataset_util.load_datasets_portions(df)

    print(f"Dataset rows with null values: {len(test_set)}")
    null_values = pd.DataFrame(test_set.isna().stack()[test_set.isna().stack()].index.tolist())
    null_values[2]=["?"]*len(null_values)
    null_values[0]=null_values[0].apply(lambda x: x+1)

    print(f"Null values count: {len(null_values)}")
    ...
    if not time_limit_reached:
        ...
        imputation_result_df.columns = ["row","column", "to_impute", "imputed"]
        imputation_result_df["row"]=imputation_result_df["row"].apply(lambda x: x-1)
        del imputation_result_df["to_impute"]
